...
export function useSurveyForm(surveyId: string | null, isPreview: boolean) {
  ...
  const submitForm = async (navigate: (path: string) => void) => {
    ...
    try {
      ...
      // Insert the main survey response using helper function
      const { data: responseData, error: responseError } = await insertIntoTable('survey_responses', {
      ...
      console.log('Survey response created with ID:', (responseData as any).id);
      ...
      if (customResponses.length > 0) {
        const customResponsesPayload = customResponses.map(([questionId, answer]) => ({
          response_id: (responseData as any).id,
        ...
        const { error: customError } = await insertIntoTable('custom_question_responses', customResponsesPayload);
